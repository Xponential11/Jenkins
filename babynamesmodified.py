...
# Year
  match = re.search(r'Popularity\sin\s(.)',t)
  if not match:
...
# Extracting Data from Tuples
  tuples = re.findall(r'<td>(\d+)</td><td>(\w+)</td><td>(\w+)</td>',t) #(rank , boy_name , girl_name)

#Dict for storing data
  names_to_rank={}
  for rank in tuples:
    (rank, boy_name, girl_name) = rank # Unpacking Tuple in required values
    if boy_name not in names_to_rank:
      names_to_rank[boy_name]= rank
    if girl_name not in names_to_rank:
      names_to_rank[girl_name]= rank

  names_sorted = sorted(names_to_rank)

#Building Result Format
